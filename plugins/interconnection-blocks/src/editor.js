 ...
 * @param {Function} [before]   Function to run before updating modules.
 * @param {Function} [after]    Function to run after updating modules.
 */
...
const autoload = ( {
	...
	const loadModules = () => {
		...
		context.keys().forEach( key => {
			const module = context( key ) || context[ key ];
...
// Load all filter index files.
autoload( {
	getContext: () => require.context( './filters', true, /index\.js$/ ),
	register: ( { filters } ) => filters.forEach(
		( { hook, namespace, callback, priority = 10 } ) =>
			addFilter( hook, namespace, callback, priority )
	),
	unregister: ( { filters } ) => filters.forEach(
		( { hook, namespace } ) => removeFilter( hook, namespace )
	),
} );
